...
function legacyVisit(v){return (v.formVersion||1)<2;}
function ratingTone(n){return !n?'unrated':n<=3?'low':n<=6?'medium':n<=9?'high':'diamond';}
function legacyNotice(v){return legacyVisit(v)?`<p class="legacy-visit" role="note">${t('Ten wpis pochodzi sprzed zmian formularza. Edytuj go, sprawdź ocenę w nowej skali 1–10 i zapisz ponownie. Dane zostały zachowane.')}</p>`:'';}
function ratingMarkup(v){const max=legacyVisit(v)?5:10,n=Math.max(0,Math.min(max,Number(v.rating)||0));return `<div class="journal-rating rating-${legacyVisit(v)?'legacy':ratingTone(n)}" aria-label="${n?t('Ocena {n} z {max}',{n,max}):t('Bez oceny')}">${n?`<span aria-hidden="true">${'★'.repeat(n)}${'☆'.repeat(max-n)}</span> <small>${n}/${max}</small>`:`<span class="muted">${t('Bez oceny')}</span>`}</div>`;}
...
function openVisit(placeId,existing){
 ...
 visitDialog.innerHTML=`<h2>${existing?t('Edytuj wizytę'):t('Twoja wizyta')}</h2><p>${escapeHtml(p.name)}</p>${existing?legacyNotice(v):''}<form class="visit-form" id="visitForm">
 <label>${t("Data wizyty")}<input name="date" type="date" required value="${escapeHtml(v.date)}"></label>
 <fieldset class="rating-field"><legend>${t("Twoja ocena")}</legend><input type="hidden" name="rating" value="${v.rating||0}"><div class="rating-choices rating-${ratingTone(v.rating||0)}">${RATING_STEPS.map(n=>`<button type="button" data-rating="${n}" aria-label="${t("Ocena {n} z 10",{n})}" aria-pressed="${v.rating===n}">${n<=(v.rating||0)?'★':'☆'}</button>`).join('')}</div><button type="button" id="clearRating">${t("Bez oceny")}</button></fieldset>
 ...
 const chooseRating=n=>{document.querySelector('.rating-choices').className='rating-choices rating-'+ratingTone(n);document.querySelector('[name=rating]').value=n;document.querySelectorAll('[data-rating]').forEach(b=>{b.textContent=Number(b.dataset.rating)<=n?'★':'☆';b.setAttribute('aria-pressed',String(Number(b.dataset.rating)===n));});};
 ...
 document.querySelector('#visitForm').onsubmit=e=>{
  ...
  const entry={...v,formVersion:2,id:existing?.id||'',placeId,date:f.get('date'),rating:Number(f.get('rating')),weather:f.get('weather'),trailUrl:full?String(f.get('trailUrl')||'').trim():(v.trailUrl||''),notes:f.get('notes'),distance_m:full?Math.round(Number(f.get('distance'))*1000):(v.distance_m||0),duration_minutes:full?Number(f.get('duration')):(v.duration_minutes||0),elevation_gain_m:full?Number(f.get('elevation')):(v.elevation_gain_m||0),photos:[...photoDraft]};
  try{
