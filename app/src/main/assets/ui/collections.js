...
function renderCollections(){
 ...
 content.innerHTML=`<section class="collections"><div class="eyebrow">${t("Twój paszport odkryć")}</div><h2>${t("Kolekcje")}</h2><div class="collection-tabs">${[['theme',t('Tematyczne')],['peaks',t('Pasma i regiony')],['plans',t('Wycieczki')]].map(([id,label])=>`<button data-collection-mode="${id}" aria-pressed="${collectionMode===id}">${label}</button>`).join('')}</div><p class="muted">${collectionMode==='plans'?t('Zapisz miejsca w takiej kolejności, w jakiej chcesz je odwiedzić.'):t('Postęp obejmuje miejsca dostępne w obecnej bazie.')}</p><div id="collectionGrid" class="collection-grid"></div></section>`;
 ...
 if(collectionMode==='plans'){
  document.querySelector('#collectionGrid').innerHTML=`<button id="newPlan" class="primary">${t("+ Dodaj własną Wycieczkę")}</button>${allPlans().map(p=>`<button class="collection-card" data-plan="${p.id}"><span class="eyebrow">${p.editable?t('Twoja lista'):t('Propozycja')}</span><h3>${escapeHtml(p.editable?p.title:t(p.title))}</h3>${progressHtml(p.placeIds)}</button>`).join('')}`;
...
function showProposals(id){const p=catalog.places.find(p=>p.id===id&&p.country===country);if(!p)return;showDetail(()=>{const matches=allPlans().filter(g=>g.placeIds.includes(id));exploreDialog.innerHTML=detailHeader(t('Propozycja wycieczki'))+`<p>${escapeHtml(p.name)}</p><p class="muted">${t("Kolejność odwiedzin możesz dopasować do swojego wyjazdu.")}</p>${matches.length?matches.map(g=>`<button class="collection-card" data-plan="${g.id}"><h3>${escapeHtml(g.editable?g.title:t(g.title))}</h3>${progressHtml(g.placeIds)}</button>`).join(''):`<p>${t("Nie ma jeszcze zapisanej propozycji dla tego miejsca.")}</p>`}<button data-new-plan="${id}" class="primary">${t("Dodaj własną Wycieczkę")}</button>`;});}
function showPlan(id){const p=allPlans().find(p=>p.id===id);if(!p)return;showDetail(()=>{exploreDialog.innerHTML=detailHeader(p.editable?p.title:t(p.title))+`<p class="muted">${t("Wycieczka · kolejność odwiedzin")}</p>${p.description?`<p>${escapeHtml(p.editable?p.description:t(p.description))}</p>`:''}${progressHtml(p.placeIds)}<ol class="visit-order">${members(p.placeIds).map(x=>`<li><button data-detail-place="${x.id}"><strong>${escapeHtml(x.name)}</strong><small>${visited.has(x.id)?t('✓ Odwiedzone'):t('Nieodwiedzone')}</small></button></li>`).join('')}</ol><button data-edit-plan="${p.id}">${p.editable?t('Edytuj kolejność'):t('Zapisz własną wersję')}</button>${p.editable?`<button data-delete-plan="${p.id}">${t("Usuń listę")}</button>`:''}`;});}
