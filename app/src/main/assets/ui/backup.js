...
document.querySelector('#openBackup').onclick=()=>{
 backupDialog.innerHTML=`<h2>${t("Kopia Twoich wspomnień")}</h2><p>${t("Jeden plik ZIP: wizyty, oceny, odznaki, własne listy i ustawienia.")}</p><button id="exportBackup">${t("Zapisz kopię")}</button><button id="importBackup">${t("Wczytaj kopię")}</button><p class="muted">${t("Wybierasz miejsce zapisu. Aplikacja nie wysyła kopii na serwer. Plik zawiera Twoje dane — przechowuj go w wybranym przez siebie miejscu.")}</p><div id="backupStatus" role="status"></div><button id="closeBackup">${t("Zamknij")}</button>`;
