...
final class BackupController {
 ...
 synchronized void confirm(String value){if(busy||pending==null||token==null||!token.equals(value)){event("error","Wybierz ponownie plik kopii.");return;}busy=true;worker.execute(()->{try{db.mergeBackup(pending,new File(activity.getFilesDir(),"photos"));notify.accept(new JSONObject().put("type","imported").put("message","Dane zostały połączone.").put("theme",db.state("theme","dark")).put("language",db.state("language","pl")));}catch(Exception e){event("error","Nie udało się połączyć danych. Poprzednie dane zostały zachowane.");}finally{clearPending();busy=false;}});}
