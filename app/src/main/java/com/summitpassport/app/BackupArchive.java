...
public final class BackupArchive {
 ...
 public static Loaded read(InputStream input,File staging)throws Exception {
  ...
  require(manifest!=null);JSONObject data=new JSONObject(new String(manifest,StandardCharsets.UTF_8));require(data.optString("format").equals("summit-passport")&&(data.optInt("version")==1||data.optInt("version")==2||data.optInt("version")==3));
 ...
 public static void validate(JSONObject data,JSONObject catalog)throws Exception {
  require(data.optString("format").equals("summit-passport")&&(data.optInt("version")==1||data.optInt("version")==2||data.optInt("version")==3));Map<String,String> places=new HashMap<>();JSONArray all=catalog.getJSONArray("places");for(int i=0;i<all.length();i++){JSONObject p=all.getJSONObject(i);if(p.optBoolean("mapReady"))places.put(p.getString("id"),p.getString("country"));}
  ...
  for(int i=0;i<visits.length();i++){JSONObject v=visits.getJSONObject(i);require(v.getString("id").matches("[a-f0-9-]{36}")&&ids.add(v.getString("id"))&&places.containsKey(v.getString("placeId")));LocalDate.parse(v.getString("date"));require(v.getString("notes").length()<=20000&&v.getString("weather").length()<=100);
   for(String key:new String[]{"rating","distance_m","duration_minutes","elevation_gain_m"}){double n=v.optDouble(key,0);require(Double.isFinite(n)&&n>=0&&n==Math.floor(n)&&n<=(key.equals("rating")?(v.optInt("formVersion",1)==2?10:5):100000000));}
   double form=v.optDouble("formVersion",1);require(form==1||form==2);if(data.optInt("version")<3)require(form==1);
   String url=v.getString("trailUrl");require(url.length()<=4000);
  }
  if(data.optInt("version")==2||data.optInt("version")==3)require(data.has("achievements"));if(data.has("achievements"))AchievementRecords.validate(data.getJSONArray("achievements"));
