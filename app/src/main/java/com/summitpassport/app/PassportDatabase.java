...
public final class PassportDatabase extends SQLiteOpenHelper {
 public PassportDatabase(Context context) { super(context, "passport.db", null, 6); }
 ...
 @Override public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
  ...
  if(oldVersion<6){
   db.execSQL("ALTER TABLE visits RENAME TO visits_legacy");
  db.execSQL("CREATE TABLE visits (id TEXT PRIMARY KEY NOT NULL, place_id TEXT NOT NULL REFERENCES places(stable_id), visited_on TEXT NOT NULL, distance_m INTEGER NOT NULL DEFAULT 0 CHECK(distance_m >= 0), duration_minutes INTEGER NOT NULL DEFAULT 0 CHECK(duration_minutes >= 0), elevation_gain_m INTEGER NOT NULL DEFAULT 0 CHECK(elevation_gain_m >= 0), notes TEXT NOT NULL DEFAULT '', weather TEXT NOT NULL DEFAULT '', trail_url TEXT NOT NULL DEFAULT '', photos TEXT NOT NULL DEFAULT '[]', rating INTEGER NOT NULL DEFAULT 0 CHECK(rating BETWEEN 0 AND 10), form_version INTEGER NOT NULL DEFAULT 2 CHECK(form_version IN (1,2)))");
   db.execSQL("INSERT INTO visits (id,place_id,visited_on,distance_m,duration_minutes,elevation_gain_m,notes,weather,trail_url,photos,rating,form_version) SELECT id,place_id,visited_on,distance_m,duration_minutes,elevation_gain_m,notes,weather,trail_url,photos,rating,1 FROM visits_legacy");
   db.execSQL("DROP TABLE visits_legacy");
   db.execSQL("CREATE INDEX visits_place ON visits(place_id)");
  }
 }
 ...
 public synchronized JSONObject backupData()throws JSONException {return new JSONObject().put("format","summit-passport").put("version",3).put("achievements",awards()).put("createdAt",java.time.Instant.now().toString()).put("visits",visitList()).put("plans",new JSONArray(state("plans","[]"))).put("settings",new JSONObject().put("theme",state("theme","dark")).put("country",state("country","")).put("language",state("language","pl")));}
 ...
 public synchronized JSONObject saveVisit(JSONObject v)throws Exception {
  int formVersion=v.optInt("formVersion",2);if((formVersion!=1&&formVersion!=2)||v.optDouble("formVersion",2)!=formVersion)throw new IllegalArgumentException("Nieprawidłowa wersja formularza.");
  int rating=v.optInt("rating",0);if(rating<0||rating>(formVersion==1?5:10)||v.optDouble("rating",0)!=rating)throw new IllegalArgumentException("Nieprawidłowa ocena.");
  ...
  ContentValues values=new ContentValues();values.put("id",id);values.put("place_id",v.getString("placeId"));values.put("visited_on",date);values.put("weather",v.optString("weather"));values.put("trail_url",link);values.put("notes",v.optString("notes"));values.put("rating",rating);values.put("form_version",formVersion);
 ...
 public synchronized JSONArray visitList()throws JSONException {
  JSONArray result=new JSONArray();try(Cursor c=getReadableDatabase().rawQuery("SELECT id,place_id,visited_on,weather,trail_url,notes,distance_m,duration_minutes,elevation_gain_m,photos,rating,form_version FROM visits ORDER BY visited_on DESC,id DESC",null)){
   while(c.moveToNext())result.put(new JSONObject().put("id",c.getString(0)).put("placeId",c.getString(1)).put("date",c.getString(2)).put("weather",c.getString(3)).put("trailUrl",c.getString(4)).put("notes",c.getString(5)).put("distance_m",c.getInt(6)).put("duration_minutes",c.getInt(7)).put("elevation_gain_m",c.getInt(8)).put("photos",new JSONArray(c.getString(9))).put("rating",c.getInt(10)).put("formVersion",c.getInt(11)));
  }return result;
 }
