...
The script is intentionally conservative: a record is CROSS_CHECKED only when a
single nearby OSM identity has a matching normalized name and a category-compatible
OSM tag. It never edits catalog files.
"""
...
def overpass(rows,radius):
    parts=[]
    for r in rows:
        parts.append(f'nwr(around:{radius},{float(r["latitude"]):.7f},{float(r["longitude"]):.7f})["name"];')
...
for start in range(0,len(rows),a.batch_size):
    ...
    try:
        raw,endpoint=overpass(batch,a.radius)
        all_elements.extend(raw.get('elements',[]))
        print(f'OVERPASS batch {start+1}-{start+len(batch)} endpoint={endpoint} elements={len(raw.get("elements",[]))}')
    except Exception as e:
        query_errors.append({'start':start,'count':len(batch),'error':type(e).__name__})
        print(f'OVERPASS_ERROR batch {start+1}-{start+len(batch)} {type(e).__name__}')
# Deduplicate returned elements from overlapping search circles.
...
for r in rows:
    ...
    for e in elements:
        ...
        if not fits(category,tags):continue
    ...
    result=dict(r,status=status,reason=reason,matches=matches)
...
fields=list(rows[0].keys())+['osm_url','osm_type','osm_id','osm_latitude','osm_longitude','distance_m','verification_status'] if rows else []
...
for x in results:
    print(x['status'],x['name'],x['reason'])
