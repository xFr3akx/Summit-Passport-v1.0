...
for i,row in enumerate(rows,1):
 name=row['name'];cat=row['category'];status='REVIEW';reason='no offline coordinate candidate';matches=[]
 lat=row.get('latitude','').strip();lon=row.get('longitude','').strip();meta={}
 ...
 if lat and lon and geonames_id:
  try:
   matches=[{'name':name,'lat':float(lat),'lon':float(lon),'osm_type':'GEONAMES','osm_id':geonames_id,'osm_key':'geonames','osm_value':feature_code,'state':row.get('region',''),'county':None,'city':None}]
   status='CANDIDATE';reason='existing GeoNames coordinate_candidate from catalog_working'
