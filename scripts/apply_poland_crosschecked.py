...
assert len(ids)==len(set(ids)), 'Duplicate stable_id in cross-checked input'
...
for row in rows:
    ...
    lat=float(row['osm_latitude']);lon=float(row['osm_longitude']);distance=float(row['distance_m'])
    assert 45<=lat<=56 and 5<=lon<=25 and distance<=500
    ...
    w['verification_status']='cross_checked_osm_geonames'
    w['coordinate_role']=role
    w['coordinate_source']={
        'url':url,
        'provider':'Overpass / OpenStreetMap',
        'retrieved_on':today,
        'method':'Exact normalized name and compatible OSM category; OSM identity within 500 m of an existing GeoNames coordinate candidate; not field surveyed.'
    }
    w['coordinate_review']=f'Automated OSM/GeoNames cross-check; OSM identity {distance:.1f} m from {row.get("source_id") or "GeoNames candidate"}.'
    # Keep source-derived identity/category metadata intact; category in working is only added when absent.
    if not w.get('category'):w['category']=row['category']
    r['lat']=lat;r['lon']=lon;r['mapReady']=True;r['source']=url;r['coordinateRole']=role
    changes.append({'stable_id':sid,'name':r['name'],'category':r['category'],'lat':lat,'lon':lon,'source':url,'distance_m':distance})
...
assert {r['stable_id'] for r in wrecords}==set(widx)
assert {r['id'] for r in rplaces}==set(ridx)
summary={
    'applied':len(rows),'stable_ids_preserved':True,'record_counts_preserved':True,
    'working_map_ready_before':old_working_ready,'working_map_ready_after':new_working_ready,
    'runtime_pl_map_ready_before':old_runtime_pl,'runtime_pl_map_ready_after':new_runtime_pl,
    'changes':changes
}
