#!/usr/bin/env python3
import json,pathlib
R=pathlib.Path(__file__).resolve().parents[1];O=R/'data/next100_broad'
...
print(f"PASS next100 broad: {len(ready)} READY, {len(review)} REVIEW, {len(dups)} DUP; cumulative staged={len(changed)}; production unchanged")
