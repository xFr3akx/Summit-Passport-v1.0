#!/usr/bin/env python3
import json,math,pathlib,re,time,unicodedata,urllib.parse,urllib.request
...
def photon(q):
    ...
    req=urllib.request.Request(url,headers={'User-Agent':'SummitPassportManual54/1.0'})
    with urllib.request.urlopen(req,timeout=20) as r: return json.load(r).get('features',[])
...
def score(r,c):
    ...
    n=norm(r['name']); cn=norm(c['name']); s=0
    parts=[norm(x) for x in re.split(r'[/—-]',r['name']) if norm(x)]
    if cn==n:s+=8
    elif any(cn==p for p in parts):s+=7
    elif cn and (cn in n or n in cn):s+=5
    elif any(p and (p in cn or cn in p) for p in parts):s+=4
    else:
        ov=len(toks(r['name']) & toks(c['name'])); s+=min(3,ov)
    context=' '.join([c['state'],c['county'],c['city'],c['district']])
    ov=len(toks(r.get('area','')) & toks(context)); s+=min(5,ov*2)
    bad={'street','house','postcode'}
    if str(c['type']).lower() in bad:s-=4
    if c['key'] in ('highway','addr'):s-=4
    return s

def osm_url(c):
    t={'N':'node','W':'way','R':'relation','node':'node','way':'way','relation':'relation'}.get(str(c['osm_type']),None)
...
for r in manual['results']:
    if r['finalStatus'] not in ('READY','REPRESENTATIVE'): continue
    out={'id':r['id'],'name':r['name'],'category':r['category'],'area':r['area'],'requestedStatus':r['finalStatus']}
    chosen=r.get('chosen')
    if chosen and chosen.get('lat') is not None:
        ...
        for q in queries:
            try:
                for f in photon(q):
                    c=cand(f); key=(c['osm_type'],str(c['osm_id']),round(c['lat'] or 0,6),round(c['lon'] or 0,6)); pool[key]=c
        ...
        if not ranked or ranked[0][0]<5:
            out.update(status='REVIEW',reason='no strong representative candidate',errors=errs);results.append(out);continue
        ...
        if second>=top[0]-1 and (ranked[1][1]['osm_id']!=top[1]['osm_id'] or ranked[1][1]['osm_type']!=top[1]['osm_type']):
            out.update(status='REVIEW',reason=f'ambiguous representative candidates {top[0]} vs {second}',candidates=[x[1] for x in ranked[:3]],errors=errs);results.append(out);continue
        c=top[1]; source=osm_url(c)
        if not source:
            out.update(status='REVIEW',reason='candidate has no OSM identity',candidate=c);results.append(out);continue
        role='representative_point_not_entrance'; conf='photon_unique_representative'
    # collision gate
    near=[]
    for p in existing:
    ...
    out.update(status='STAGE',lat=c['lat'],lon=c['lon'],source=source,coordinateRole=role,confidence=conf,chosen=c)
    results.append(out)

stage=[x for x in results if x['status']=='STAGE']
newcat=json.loads(json.dumps(cat))
newby={p['id']:p for p in newcat['places']}
for x in stage:
    p=newby.get(x['id'])
